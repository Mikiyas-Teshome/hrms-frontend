...
export function InsuranceFormFields({
  ...
  const assignmentOptions = Object.values(InsuranceAssignment).map((val) => ({
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="p-6 bg-white dark:bg-zinc-950/40 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-2 py-2 border-t border-slate-100 dark:border-zinc-900 pt-4">
            <Switch
              ...
              onCheckedChange={(checked) => setValue('hasDependentsCoverage', checked, { shouldDirty: true })}
            />
          ...
          <div className="flex flex-col gap-3 border-t border-slate-100 dark:border-zinc-900 pt-4">
            <Label className="text-sm font-semibold text-slate-700 dark:text-zinc-300">Included services</Label>
            <div className="flex flex-wrap gap-x-6 gap-y-3">
              {Object.values(InsuranceIncludedService).map((service) => (
                <div key={service} className="flex items-center gap-2">
                  <Checkbox
                    id={`service-${service}`}
                    checked={includedServices.includes(service)}
                    onCheckedChange={(checked) =>
                      handleCheckboxChange('includedServices', service, !!checked)
                    }
                  />
                  <Label htmlFor={`service-${service}`} className="text-sm font-normal capitalize text-slate-600 dark:text-zinc-400 cursor-pointer">
      ...
      <div className="border border-slate-200/80 dark:border-zinc-800 rounded-xl overflow-hidden bg-white dark:bg-zinc-950/20 shadow-sm shadow-slate-100/50 dark:shadow-none animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 bg-white dark:bg-zinc-950/40">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="minTenureMonths" className="text-sm font-semibold text-slate-700 dark:text-zinc-300">
              Minimum tenure
            </Label>
            <Input
              type="number"
